...
class TD3(object):
    def __init__(self, params):
        ...
        self.agent_params['target_noise'] = self.params['target_noise']
        self.agent_params['noise_clip'] = self.params['noise_clip']
        self.agent_params['policy_delay']= self.params['policy_delay']
    ...
    def run(self):
        ...
        for t in range(self.total_envsteps):
            ...
            self.agent.replay_buffer.insert(o, a, r, next_o, d)
            ...
            if t >= self.params['update_after'] and t % self.params['update_freq'] == 0:
                ...
                for j in range(self.params['update_freq']):
                    batch = self.agent.replay_buffer.sample(self.params['batch_size'])
                    train_log = self.agent.train(batch, j)
                    if train_log['PiLoss']:
                        all_logs['QLoss'].append(train_log['QLoss'])
                        all_logs['PiLoss'].append(train_log['PiLoss'])
                        all_logs['Q1_values'].append(np.mean(train_log['Q1_values']))
                        all_logs['Q2_values'].append(np.mean(train_log['Q2_values']))
                    else:
                        all_logs['QLoss'].append(train_log['QLoss'])
    ...
    def perform_logging(self, env, epoch, test_policy, train_stats, all_logs):
        ...
        logs["Q1_values"] =np.mean(all_logs["Q1_values"])
        logs["Q2_values"] =np.mean(all_logs["Q2_values"])
